...
export default class ValueDelta {
  ...
  constructor(ops: ValueOp[] = []) { this.#ops = [...ops ]; }

  retain(length: number): this {
    this.#ops.push(<ValueDeltaRetain>{kind : 'retain', retain : length});
    return this;
  }

  insert(content: string): this {
    this.#ops.push(<ValueDeltaInsert>{kind : 'insert', insert : content});
    return this;
  }

  remove(length: number): this {
    this.#ops.push(<ValueDeltaRemove>{kind : 'remove', remove : length});
    return this;
  }
