 ...
 *
 * @param {any} params - The action dependencies.
...
export function actions(params: {
  ...
  return {
    fetchNotes({commit}, payload: { noteIDs: string[] }): Promise<void> {
      commit('notesLoading', {noteIDs: payload.noteIDs});
      return params.api
          .getNotes('', payload.noteIDs)
          .then((notes: Partial<NoteRecord>[]) => {
            commit('notesLoaded', {notes});
          })
          .catch(() => {
            commit('notesLoadError', {noteIDs: payload.noteIDs});
          });
    },
  };
}
