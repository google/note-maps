...
func RandomID() note.ID {
	return note.ID(strconv.FormatUint(rand.Uint64(), 10))
}
...
func (n BrokenNote) GetID() note.ID { return n.ID }

// GetValue always returns n.Err.
func (n BrokenNote) GetValue() (string, note.GraphNote, error) { return "", nil, n.Err }

// GetContents always returns n.Err.
func (n BrokenNote) GetContents() ([]note.GraphNote, error) { return nil, n.Err }

// GetTypes always returns n.Err.
func (n BrokenNote) GetTypes() ([]note.GraphNote, error) { return nil, n.Err }
...
func TestLoader(t testing.TB, l note.Loader) {
	...
	ids := []note.ID{RandomID(), RandomID(), RandomID()}
	ns, err = l.Load(ids)
	if err != nil {
		t.Error(err)
	...
	ids = []note.ID{RandomID(), RandomID(), RandomID()}
	for i := range ids {
		bads := append([]note.ID{}, ids...)
		bads[i] = note.EmptyID
		_, err := l.Load(bads)
		if !errors.Is(err, note.InvalidID) {
			t.Error("with bad ID at", i, "expected", note.InvalidID)
		}
	}
}
