...
func TestBreakingLoader_Load(t *testing.T) {
	var l = BreakingLoader{Loader: note.EmptyLoader, Err: errors.New("testing")}
...
func TestBrokenNote(t *testing.T) {
	...
	var n note.GraphNote = BrokenNote{id, expectErr}
	if n.GetID() != id {
...
func expectBroken(t *testing.T, n note.GraphNote, expectErr error) {
...
func TestBrokenNoteLoader(t *testing.T) {
	var l note.Loader = &BrokenNoteLoader{errors.New("testing broken note loader")}
...
type loaderFunc func([]note.ID) ([]note.GraphNote, error)

func (l loaderFunc) Load(ids []note.ID) ([]note.GraphNote, error) { return l(ids) }

func TestTestLoader_acceptsEmptyLoader(t *testing.T) {
	TestLoader(t, note.EmptyLoader)
}

func TestTestLoader(t *testing.T) {
	good := note.EmptyLoader
	for _, test := range []struct {
		Title  string
		L      note.Loader
		Expect string
	}{
		{"accepts empty loader", note.EmptyLoader, ""},
		{
			"verifies empty request gets no error",
			loaderFunc(func(ids []note.ID) ([]note.GraphNote, error) {
				if len(ids) == 0 {
		...
		},
		{
			...
			loaderFunc(func(ids []note.ID) ([]note.GraphNote, error) {
				if len(ids) == 0 {
					return []note.GraphNote{note.EmptyNote("0")}, nil
				}
				return good.Load(ids)
			}),
		...
		},
		{
			...
			loaderFunc(func(ids []note.ID) ([]note.GraphNote, error) {
				if len(ids) == 1 {
					return []note.GraphNote{note.EmptyNote("nope")}, nil
				}
				return good.Load(ids)
			}),
			"expected .* got nope\n",
